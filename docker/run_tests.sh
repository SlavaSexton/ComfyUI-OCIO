...
#   1. Run every standalone tests/test_*.py (exit non-zero on any failure).
#   2. Boot ComfyUI headless and assert every OCIO node registers (proves the pack imports in ComfyUI).
#   3. Optional: tests/accuracy regression suite when RUN_ACCURACY=1.
set -uo pipefail
...
# The pack's own tests/test_*.py target the built-in ACES 2.x config (they reference names like
...
if [ "${RUN_ACCURACY:-0}" = "1" ]; then
    ...
    cd "$PACK_DIR" || exit 2
    python tests/accuracy/gen_fixtures.py || fail=1
    for m in tests/accuracy/measure_*.py; do
        echo "----- $m -----"
        python "$m" || fail=1
    done
fi
