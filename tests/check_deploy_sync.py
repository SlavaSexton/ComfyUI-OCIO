...
tests/, docker/, .github/ - see that file's own comment for why). This checker exists for a different
distribution path: a git clone or a hand-copied folder under custom_nodes, which is how the pack actually
lands on a dev/render box, and on that path the tests/ test scripts and docs genuinely are what's on disk
...
Usage:
    python tests/check_deploy_sync.py --path "C:\\ComfyUI\\custom_nodes\\ComfyUI-OCIO"
    COMFYUI_OCIO_INSTALL_PATH="C:\\ComfyUI\\custom_nodes\\ComfyUI-OCIO" python tests/check_deploy_sync.py
    python tests/check_deploy_sync.py --repo-path "C:\\dev\\ComfyUI-OCIO" --path "C:\\ComfyUI\\custom_nodes\\ComfyUI-OCIO"
...
def default_repo_root() -> Path:
    """The repository is wherever this script itself lives - one directory up from tests/."""
