"""Regression: OCIO Write's input is called `input_colorspace`, like everywhere else (run: python tests/test_input_colorspace_rename.py).
...
tests/ scripts, docker/, anything driving the node from Python - are not prompts and never went through that
...
def check_write_still_answers_to_the_old_name(io):
    ...
    assert OLD in sig and sig[OLD].default is None, (
        f"write() no longer accepts `{OLD}`; the repo's own tests/ and docker/ scripts call it by that name")
