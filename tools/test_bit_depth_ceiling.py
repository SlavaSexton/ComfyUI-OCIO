...
Run:  python tools/test_bit_depth_ceiling.py     (no pytest, no ComfyUI server, no GPU; ffmpeg/ffprobe needed
...
encoder table:
  ...
  1. every video_codec: encode a short clip THROUGH THE NODE, read it back with ffprobe, assert the pix_fmt
     ...
     compared against the front-end label the way tools/test_codec_ext_parity.py does it - see the note above
...
WHY THIS IS NOT A DUPLICATE of tools/test_codec_ext_parity.py's depth section. That file (a) hand-assembles
...
def measured_bits(info):
    ...
    tools/test_codec_ext_parity.py already uses. Needed because this build's ffprobe reports NO
...
# uint8 for a real 16-bit RGB PNG (documented in tools/test_metadata_all_formats.py's _png_bitdepth docstring,
...
# tools/test_metadata_all_formats.py check_tiff()).
...
# NOT A FULL DUPLICATE of tools/test_write_output.py, which already puts an out-of-range pixel (-0.25, 1.75, 3.5)
