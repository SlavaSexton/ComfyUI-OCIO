"""Regression: the `view` list is narrowed to what the pair can actually render (run: python tools/test_view_narrowing.py).
...
under a green gate; see tools/test_node_buttons_present.py). So `viewsForCrossing` is lifted out of the file
